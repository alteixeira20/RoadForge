...
Examples:
    ...
    python -m api.scripts.purge_retention --as-of 2026-08-10T18:00:00Z
...
def _report(
    ...
    *,
    ...
    payload = {
        "mode": mode,
        "as_of": reference_time.astimezone(timezone.utc).isoformat().replace("+00:00", "Z"),
        "policy": {
...
async def _run(args: argparse.Namespace) -> int:
    ...
    reference_time = args.as_of or datetime.now(timezone.utc)

    async with async_session_factory() as db:
        plan = await build_retention_plan(db, policy, now=reference_time)
        _report(
            "execute-preview" if args.execute else "dry-run",
            policy,
            plan.counts(),
        ...
        _report(
            ...
            policy,
            {
                ...
                "stale_task_claims": result.stale_task_claims,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="TIMESTAMP",
        help=(
            "ISO-8601 UTC/offset timestamp used for all cutoffs. The emitted as_of value can "
            "be reused to reproduce a plan exactly."
        ),
    )
    parser.add_argument(
        "--execute",
