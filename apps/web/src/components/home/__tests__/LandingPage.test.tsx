...
describe('landing page refinement', () => {
  it('starts the Hero with its heading and renders one metadata row after the preview', () => {
    ...
    expect(container.textContent).toContain('Hosted demo')
    expect(container.textContent).toContain('larger-team use')
    expect(container.textContent).toContain('Fork / self-host')
    expect(container.textContent).toContain('Team sharing is in progress — available soon')
  ...
  it('keeps six feature cards with icon-and-heading rows', () => {
    ...
    expect(container.textContent).toContain('Fork and self-host')
    expect(container.textContent).toContain('larger-team')
  })
  ...
  it('places the flow strip after the three How it works cards exactly once', () => {
    ...
    expect(container.textContent).toContain('without choosing your next action')
    expect(container.textContent).toContain('fork and self-host RoadForge')
  ...
  it('keeps landing sections in normal flow with responsive grids, bottom spacing, and sticky-header offset rules', () => {
    ...
    expect(css).toContain('grid-template-rows: minmax(0, 1fr) auto auto auto auto auto')
    expect(css).toContain('--hero-preview-gap: clamp(20px, 2.5vh, 30px)')
