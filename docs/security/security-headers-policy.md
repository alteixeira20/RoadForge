...
RoadForge / Anvilary Roadmaps is accountless. Access is controlled by role-scoped invite links, optional roadmap passwords, and participant session tokens stored in browser storage. A strict security headers policy should reduce browser-side attack surface without changing that collaboration model.
